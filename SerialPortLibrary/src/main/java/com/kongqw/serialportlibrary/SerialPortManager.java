...
public class SerialPortManager extends SerialPort {
    ...
    public static  byte[] dirToBytes(String dirsString){
        if(TextUtils.isEmpty(dirsString)) {
            return null;
        }
        String[] data  = dirsString.replace(" ", "").split(",");
    ...
    public  void lineOrder(String dirsString){
        byte[] bytes = dirToBytes(dirsString);
        boolean sendBytes = sendBytes(CommandControl.moveLine(bytes));
        Log.i(TAG, sendBytes?"新路线发送成功":"新路线发送失败");

    }

    public boolean stop(){
        boolean sendBytes = sendBytes(CommandControl.stop());
